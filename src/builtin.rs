...
use std::fs::{read_dir, ReadDir, metadata, Metadata, File, OpenOptions, remove_file, remove_dir_all};
    ...
    match &commands.first().unwrap_or(&String::new())[..] {
        "ls" => {
            if let Err(e) = list_files_builtin(commands) {
                eprintln!("\x1b[38;2;255;0;0mError: Could not list contents\n{}\x1b[0m", e);
                return Err(e);
            }
            Ok(true)
        }
        "rm" => {
            if let Err(e) = file_remove_builtin(commands) {
                eprintln!("\x1b[38;2;255;0;0mError: Could not remove file/directory\n{}\x1b[0m", e);
                return Err(e);
            }
            Ok(true)
        }
        "touch" => {
            if let Err(e) = touch_builtin(commands) {
                eprintln!("\x1b[38;2;255;0;0mError: Could not create file\n{}\x1b[0m", e);
                return Err(e);
            }
            Ok(true)
        }
        "cd" => {
            if let Err(e) = change_dir_builtin(commands) {
                eprintln!("\x1b[38;2;255;0;0mError: Could not change directories\n{}\x1b[0m", e);
                return Err(e);
            }
            Ok(true)
        }
        ...
        "history" => {
            if let Err(e) = history_builtin(commands, &mut history) {
                eprintln!("\x1b[38;2;255;0;0mError: Could not display history\n{}\x1b[0m", e);
                return Err(e);
            }
            Ok(true)
        }
        "clear" => {
            if let Err(e) = clear_builtin(commands) {
                eprintln!("\x1b[38;2;255;0;0mError: Could not clear the screen\n{}\x1b[0m", e);
                return Err(e);
            }
            Ok(true)
        }
        "cat" => {
            if let Err(e) = display_file_contents(commands) {
                eprintln!("\x1b[38;2;255;0;0mError: Could not display file contents\n{}\x1b[0m", e);
                return Err(e);
            }
            Ok(true)
        }
...
fn list_files_builtin(args: &[String]) -> Result<(), Error> {
    ...
    if args.len() == 1 {
        ...
        print_directory_contents(paths)?;
        println!()
    }
    // If ls was given multiple directories as an argument
        ...
        for directory in directories {
            ...
            if valid_directory == true {
                ...
                let rhs_width: usize = if (lhs_width % 2) == 0 { lhs_width + 1 } else { lhs_width };

                // Print contents of directory
                println!("{}[{}]{}", "-".repeat(lhs_width), directory, "-".repeat(rhs_width));
                print_directory_contents(paths)?;
                println!()
            }
            else {
...
fn print_directory_contents(paths: ReadDir) -> Result<(), Error> {
    ...
    for path in paths {
        ...
        if PathBuf::from(&path_str).is_dir() {
            ...
            let directory_name: String = "\x1b[38;2;42;125;211m".to_owned() + &path_str
                .split("/")
                .collect::<Vec<&str>>()[1].to_owned() + "/\x1b[0m";

            println!("{:<19}  {:<41}", file_modified_time
                .format("%m-%d-%Y %I:%M %p"), directory_name);
        }
        else {
            println!("{:<19}  {:<41}", file_modified_time
                .format("%m-%d-%Y %I:%M %p"), path_str
                .split("/")
                .collect::<Vec<&str>>()[1]);
        }
    }
...
fn touch_builtin(args: &[String]) -> Result<(), Error> {
    ...
    for file_path in &args[1..] {
        ...
        if file.exists() {
            let file_to_change: File = OpenOptions::new()
                            .append(true)
                            .write(true)
                            .open(file)?;
...
fn change_dir_builtin(args: &[String]) -> Result<(), Error> {
    // If no arguments are given
    if args.len() == 1 {
        return Err(Error::new(ErrorKind::InvalidInput, "Usage: cd <directory path>"))
    }
...
fn display_file_contents(args: &[String]) -> Result<(), Error> {
    ...
    if args.len() == 1 || args.len() > 2 {
        return Err(Error::new(ErrorKind::InvalidInput, "Usage: cat <path to file>"))
    }

    // Given file to display to the screen
    let display_file: File = OpenOptions::new()
        .read(true)
        .open(&args[1])?;
